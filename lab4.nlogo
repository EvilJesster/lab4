...
to setup
  ca
  resize-world -64 64 -64 64
  set-patch-size 4
  ifelse (conorrug = "con") ;this would be a single pole double throw switch
  [
...
to born
  set ali? true
  set pcolor (5 + ((random 13) * 10)) ; I had to show off, but a simple random 255 would have a chance to make it black
end
...
to deathorlife
  ...
  [
    ask patches[
    set alineigh count neighbors with [ali?]]
  ask patches
     [ifelse alineigh = 3
      [born]
      [ if alineigh != 2
